...
def main(args):
  ...
  try:
    ...
    engagePosition = [0.567, -0.0623, 0.015] # if the engage position is too loose, the controller will fail (probably because of the horizontal flow)
    ...
    while not suctionSuccessFlag:   # while no success in grasp, run controller until success or timeout
      ...
      #currPose.pose.position.z = currPose.pose.position.z + 0.005
      ...
      elif (all(np.array(P_array) > P_vac)):
        print("Suction controller failed!")
        # stop at the last pose
        rtde_help.stopAtCurrPoseAdaptive()
        msg.state, msg.pwm = PUSH_STATE, 60
        print("I am pushing")
        PushPull_pub.publish(msg)
        measuredCurrPose = rtde_help.getCurrentPose()
        currPose = search_help.get_PoseStamped_from_T_initPose(T_move, measuredCurrPose)
        currPose.pose.position.z = currPose.pose.position.z + 0.010
        rtde_help.goToPoseAdaptive(currPose)
