...
def main(args):
    ...
    try:
        ...
        # 고정된 deformation 값 (6mm)
        fixed_deformation = 6.0  # mm
        args.deformation = fixed_deformation
        
        # 반지름 범위: 38mm부터 0.5mm씩 증가하여 42mm까지
        radii = np.arange(38.0, 42.5, 0.5)  # [38.0, 38.5, 39.0, ..., 42.0]
        
        # 각도 범위: 0도부터 5도씩 증가하여 360도까지 (반시계 방향)
        angles_deg = np.arange(0, 360, 5)  # [0, 5, 10, ..., 355]
        ...
        for radius_mm in radii:
            ...
            for angle_idx, angle_rad in enumerate(angles_rad):
                ...
                engagePosition_circle[2] = disengagePosition_circle[2] - fixed_deformation * 1e-3  # 6mm만큼 내려감
                ...
                args.deformation = fixed_deformation  # deformation (mm)
                
                dataLoggerEnable(False)
                file_help.saveDataParams(args,
                    appendTxt=f'Gia_circular_radius_{radius_mm}mm_deformation_{fixed_deformation}mm_angle_{angle_deg}deg_material_{args.material}')
