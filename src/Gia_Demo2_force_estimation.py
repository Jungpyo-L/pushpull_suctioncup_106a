...
def main(args):
    ...
    try:
        ...
        if args.zHeight:
            # Use the first deformation (3.0) to find engage_z
            engage_z = disengagePosition_init[2] - 3.0*1e-3
        else:
        ...
        disengagePosition[0] += args.xoffset*0.001
        
        # ====== deformation 반복: 3부터 8까지 0.5mm 간격 ======
        deformations = np.arange(3.0, 8.5, 0.5)  # [3.0, 3.5, 4.0, ..., 8.0]
        
        for deformation in deformations:
            args.deformation = deformation
            # 각 deformation에 대한 engage position 계산
            engagePosition = copy.deepcopy(disengagePosition)
            engagePosition[2] = disengagePosition_init[2] - deformation*1e-3
            
            # Disengage Pose로 이동 (공중 위치)
            targetOrientation = tf.transformations.quaternion_from_euler(default_yaw, pi, 0, 'szxy')
            targetPose_disengage = rtde_help.getPoseObj(disengagePosition, targetOrientation)
            rtde_help.goToPose(targetPose_disengage)
            
            # 공중에서 push OFF 상태로 2초 대기 (안정화)
            msg.state, msg.pwm = PUSH_STATE, DUTYCYCLE_0
            PushPull_pub.publish(msg)
            syncPub.publish(SYNC_RESET)
            rospy.sleep(0.1)
            ...
            # 공중에서 push OFF로 2초 대기
            print(f"Waiting 2 seconds in air with push OFF (deformation: {deformation}mm)")
            rospy.sleep(2.0)
            
            # Engage Pose로 이동 (deformation 위치)
            targetOrientation = tf.transformations.quaternion_from_euler(default_yaw, pi, 0, 'szxy')
            targetPose_engage = rtde_help.getPoseObj(engagePosition, targetOrientation)
            rtde_help.goToPose(targetPose_engage)
            rospy.sleep(0.2)
            
            # === PUSH OFF 상태로 2초 동안 normalForce 측정 ===
            msg.state, msg.pwm = PUSH_STATE, DUTYCYCLE_0
            PushPull_pub.publish(msg)
            rospy.sleep(0.1)
            ...
            # 파일 저장: "push off, deformation 숫자"
            file_help.saveDataParams(args,
                appendTxt=f'push_off_deformation_{deformation}_xoffset_{args.xoffset}')
            
            file_help.clearTmpFolder()
            rospy.sleep(0.1)
            
            # === PUSH ON 상태로 2초 동안 normalForce 측정 ===
            ...
            # 파일 저장: "push on, deformation 숫자"
            file_help.saveDataParams(args,
                appendTxt=f'push_on_deformation_{deformation}_xoffset_{args.xoffset}')
            
            file_help.clearTmpFolder()
            P_help.stopSampling()
            rospy.sleep(0.1)
            
            print(f"Completed measurement for deformation: {deformation}mm")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--xoffset', type=int, default=-6)
